...
Report links are tied to the current run ID. Each run is stored under `data/runs/<run-id>/`, so opening Text report or JSON after a new calculation cannot show another run's values. The shared `/api/output.txt` and `/api/output.json` endpoints remain compatibility fallbacks for older saved results.
...
Engineering mode ranks the complete feasible Pareto set before retaining the best 32 results. It minimizes the equal-weight Euclidean distance of normalized loss, active mass and material cost index. Its configurable physical-fit and objective guardrails are in `data/config.txt`. **Textbook/MATLAB mode** uses the lecturer's core-type acceptance gate (actual L/(D-d) > 2.5 and <= 4, I0/I2 < 3%, LV/HV axial slack > 7 mm and 0.85-PF full-load efficiency > 98.5%) and reports four independent extrema, just as the textbook does. It deliberately does not impose engineering thermal, mass, clearance or actual-current-density limits. Select the profile in Optimal configuration; the report records which gate was used. Empty-feasible searches retain their samples and explicit diagnostic recommendations.

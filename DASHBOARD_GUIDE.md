...
After the search, choose **Run this design** beneath the objective you need: maximum efficiency, minimum kg/kVA, minimum I₀/I₂ or minimum tank volume. This immediately recalculates all six sections in Explore using the exact configuration that produced that variant. In **Engineering / production** mode, **Run balanced design** applies the Pareto recommendation. In **Textbook / MATLAB** mode there is intentionally no single balanced button: the four independent criterion winners are the result. The profile and acceptance gate are printed in the report.

Text report and JSON links use the current run ID, so they open the report generated by the result currently displayed. The old shared endpoints are used only before a run ID is available.
...
The Engineering balanced recommendation uses the full feasible Pareto frontier for equal-weight normalized loss, active mass and material-cost ranking. Textbook/MATLAB mode instead applies only the lecturer's five core-type checks and keeps the four objective winners separate. Neither mode guarantees Kenya Power benchmark compliance or manufacturing approval. If no attempt passes the selected gate, samples remain visible and recommendations explicitly identify the calculated diagnostics.
